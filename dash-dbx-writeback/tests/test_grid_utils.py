...
def test_numeric_column_definitions(sample_dataframe):
    ...
    numeric_cols = [col for col in column_defs if col['field'] in ['numeric_int', 'numeric_float']]
...
def test_text_column_definitions(sample_dataframe):
    ...
    # Find text columns
    text_cols = [col for col in column_defs if col['field'] in ['text', 'snake_case_text', 'null_values']]
...
def test_header_name_formatting(sample_dataframe):
    ...
    snake_case_col = next(col for col in column_defs if col['field'] == 'snake_case_text')
